...
from utils import setup_seed,read_data,training_graph,evaluate,graph2data
import argparse
import os
...
parser.add_argument('--dataset', type=str, default='Celegans', dest = 'dataset', help='dataset name')
# Celegans SMG EML YST KHN ADV LDG HPD GRQ ZWL
parser.add_argument('--seed', type=int, default=1, dest = 'seed', help='data seed')
...
def run(args):
    ...
    adj,G = read_data(args.dataset)
    G2, adj_train, train_edges, test_edges, neighbor_list, node_attr = training_graph(G, args.testing_percentage,args.hop)
